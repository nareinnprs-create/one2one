...
class Brutal(One2OneTool):
    ARCHIVED = True
    ARCHIVED_REASON = "Unmaintained — no commits since 2020-03"
    TITLE = "Brutal"
    SUPPORTED_OS = ["linux"]
    DESCRIPTION = (
...
class MSFVenom(One2OneTool):
    ARCHIVED = True
    ARCHIVED_REASON = "Unmaintained — no commits since 2021-05"
...
class Enigma(One2OneTool):
    ARCHIVED = True
    ARCHIVED_REASON = "Upstream repo archived by author (last commit 2021-08)"
    TITLE = "Enigma"
    SUPPORTED_OS = ["linux"]
...
class PayloadCreatorTools(One2OneToolsCollection):
